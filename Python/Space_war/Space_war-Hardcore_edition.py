...
pygame.display.set_caption("Space War: Hardcore edition")
...
spacewar_text = centerFont.render("Space War: Hardcore edition",True,blue)
...
class Boss:
    ...
    def fire(self):
        ...
        if self.b_spawndelay == self.b_maxtime:
            self.b_spawndelay = 0
            self.b_maxtime = randrange(20,80,10)
            new_enemybullet = Enemybullet(self.rect.left, self.rect.centery)
            enemy_bullets.append(new_enemybullet)
...
while not closed:
    ...
    if not game_started:
        ...
        if player.score >= player.reach_score:
            ...
            if player.level == 3:
                player.reach_score = 2000

            if player.level == 4:
                player.reach_score = 3000
        ...
        if player.levellock == 0:
            ...
            if player.level == 3:
                if a_spawndelay == 80:
                    a_spawndelay = 0
                    new_asteroid = Asteroid(which=randrange(1, 3), size=randrange(4, 12), speed=randrange(3, 5),
                                            ytime=randrange(0, 6))
                    asteroids.append(new_asteroid)
            ...
            if player.level == 4:
                if a_spawndelay == 80:
                    a_spawndelay = 0
                    new_asteroid = Asteroid(which=randrange(1, 3), size=randrange(4, 11), speed=randrange(3, 6),
                                            ytime=randrange(0, 6))
                    asteroids.append(new_asteroid)
            ...
            if player.level == player.maxlevel:
                if a_spawndelay == 180:
                    a_spawndelay = 0
                    new_asteroid = Asteroid(which=randrange(1, 3), size=randrange(4, 13), speed=randrange(2, 6),
                                            ytime=randrange(0, 6))
                    asteroids.append(new_asteroid)
